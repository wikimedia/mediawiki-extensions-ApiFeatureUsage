...
class ApiFeatureUsage_HTMLDateRangeField extends ApiFeatureUsage_HTMLDateField {
	...
	function validate( $value, $alldata ) {
		...
		if ( !empty( $this->mParams['absolute'] ) ) {
			if ( $value[0] === '' || $value[1] === '' ) {
				return $this->msg( 'apifeatureusage-htmlform-daterange-error-partial' )->parse();
			}
			...
			if ( $opts ) {
				if ( !in_array( $value[1], $opts ) ) {
					return $this->msg( 'apifeatureusage-htmlform-daterange-days-badoption' )->parseAsBlock();
				}
			} else {
				if ( !preg_match( '/^(\+|\-)?\d+$/', trim( $value[1] ) ) ) {
					return $this->msg( 'apifeatureusage-htmlform-daterange-days-invalid' )->parseAsBlock();
				}
				...
				if ( $min > $value[1] ) {
					return $this->msg( 'apifeatureusage-htmlform-daterange-days-toolow' )->numParams( $min )
						->parseAsBlock();
				}
				...
				if ( isset( $this->mParams['max-days'] ) ) {
					...
					if ( $max < $value[1] ) {
						return $this->msg( 'apifeatureusage-htmlform-daterange-days-toohigh' )->numParams( $max )
							->parseAsBlock();
					}
				}
			}
	...
	function getInputHTML( $value ) {
		...
		if ( !empty( $this->mParams['absolute'] ) ) {
			$msg = 'apifeatureusage-htmlform-daterange-absolute-layout';
			$oldName = $this->mName;
			$oldID= $this->mID;
			$this->mName .= '-end';
			$this->mID .= '-end';
			...
			$msg = 'apifeatureusage-htmlform-daterange-relative-layout';
			$opts = $this->getOptions();
			if ( $opts ) {
